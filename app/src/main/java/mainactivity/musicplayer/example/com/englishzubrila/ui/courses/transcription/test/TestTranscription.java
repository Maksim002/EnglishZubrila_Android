package mainactivity.musicplayer.example.com.englishzubrila.ui.courses.transcription.test;
...
public class TestTranscription extends AppCompatActivity {
    ...
    String quizData[][] = {
            ...
            {" Какой звук дает сочетание букв «ch»", " [ʧ]", " [ʃ]", "[h] ", "[s]"},
            {" Какой звук дает сочетание букв «sh»", " [ʃ]", " [ð]", "[h] ", "[s]"},
            {" Как пишется слово «['sentǝ]» ?", " centre", "Santa", " city", " candy"},
            {"В каком слове есть звук «[ʤ]»?", " giant", " get", " girl", " give"},
            {" Как обозначается долгий гласный звук в транскрипции? ", " двоеточием", "кавычками", " запятой", " восклицательным знаком "},
            {" В каком слове нет звука «[f]»?", " rhythm (ритм)", " phone (телефон)", " elephant (слон)", " wolf (волк)"},
            {" Сколько звуков в английском языке?", "44", "36", "52", "42"},
            {" Укажите дифтонг ", " [aɪ] ", " [a:] ", " [æ] ", " [θ]"},
            {" В каком слове нет звука «[eɪ]» ?", " bear ", " make ", "  day ", " lake"},
            {" В каком слове нет звука «[θ]»?", " that ", " think ", " thick ", " both "}
    };
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mActivity = TestTranscription.this;
    ...
    public String checkAnswer(View view) {
        ...
        if (alertTitle == correct) {
            LayoutInflater factory = LayoutInflater.from(TestTranscription.this);
        ...
        WindowManager.LayoutParams wmlp = dialogT.getWindow().getAttributes();

        if (alertTitle.equals(correct)){
            Objects.requireNonNull(dialogT.getWindow()).setBackgroundDrawable(new ColorDrawable(getResources().getColor(R.color.colorTrue)));
        } else {
            Objects.requireNonNull(dialogT.getWindow()).setBackgroundDrawable(new ColorDrawable(getResources().getColor(R.color.colorFalse)));
        }
        dialogT.show();
        return correct;
    }
